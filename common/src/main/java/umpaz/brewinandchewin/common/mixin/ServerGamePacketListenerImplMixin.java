    ...
    public PlayerChatMessage brewinandchewin$modifyChatMessageForServer(PlayerChatMessage message) {
        ...
        if (sender.hasEffect(BnCEffects.TIPSY.value()) && sender.getEffect(BnCEffects.TIPSY.value()).getAmplifier() >= 3) {
            ...
            for (ServerPlayer otherPlayer : player.getServer().getPlayerList().getPlayers())
                BrewinAndChewin.getHelper().sendClientbound(otherPlayer, new MakeNextPlayerChatTipsyClientboundPacket(sender.getEffect(BnCEffects.TIPSY.value()).getAmplifier(), randomSeed, 0));
